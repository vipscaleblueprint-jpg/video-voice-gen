    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (Array.isArray(data) && data.length > 0) {
                ...
                const item = data[0];
                if (item.output) generatedText = item.output;
                else if (item.text) generatedText = item.text;
                else if (item.result) generatedText = item.result;
                else {
                ...
                if (data.output) generatedText = data.output;
                else if (data.text) generatedText = data.text;
                else generatedText = JSON.stringify(data, null, 2);
            } else {
    ...
    return (
        <div className="container mx-auto px-4 py-8 max-w-4xl space-y-8 animate-fade-in">
            ...
            <div className="grid gap-8">
                    ...
                    <CardContent className="p-6">
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div className="space-y-2">
                                ...
                                <Textarea
                                    id="input-source"
                                    placeholder="e.g. https://mybusiness.com, or 'I help doctors with financial planning...'"
                                    value={inputText}
                                    onChange={(e) => setInputText(e.target.value)}
                                    className="bg-secondary/50 border-white/10 min-h-[150px] focus:ring-primary/20 transition-all font-medium"
                                />
                            </div>
                            ...
                            <Button
                                ...
                                className="w-full h-12 text-base font-semibold bg-primary text-primary-foreground hover:bg-primary/90 disabled:opacity-50 glow-primary transition-all rounded-xl"
                            >
                                {isLoading ? (
                                    <>
                ...
                {/* Output Section */}
                {result && (
                    <Card className="glass border-white/5 shadow-card animate-slide-up">
